...
│   ├── ttsService.js       # Google Cloud TTS 호출 로직
│   └── tokenService.js     # JWT 발급/검증/폐기 로직
├── images/                 # 업로드된 이미지 저장 디렉토리
├── audios/                 # TTS 생성 음성 파일 저장 디렉토리
├── .env                    # 환경변수 (Git 제외)
...
| 동화 | POST | `/tts-preview` | ✅ | 동화 내용 TTS 미리듣기 (저장 없이 스트리밍) |
...
user_id        INT NOT NULL  -- FK → users.user_id
audio_url      VARCHAR(255)  -- TTS 음성 파일 URL (실패 시 NULL)
...
### 주의사항
- OpenAI API 키 및 Google Cloud TTS는 유료 사용량에 따라 과금됩니다
- `.env` 파일과 `google-credentials.json`은 절대 Git에 커밋하지 마세요
- `images/`, `audios/` 디렉토리가 없는 경우 미리 생성해두세요
...
- **Rate Limiting**: API별 요청 횟수 제한 (동화 생성·TTS 미리듣기 1시간 3회, 동화 저장 1시간 5회, 로그인 1시간 5회, 회원가입 1시간 2회)
